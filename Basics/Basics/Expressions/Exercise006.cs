...
public class Exercise006 {
    ...
    */
    public static void UserMain() {
        ...
        while (true) {
            Console.Write("Type a positive integer number to check the bit position from {0} to {1}: ", posMinValue, posMaxValue);
            string? userInput = Console.ReadLine();

            bool isPositionValid = int.TryParse(userInput, out position) && position >= posMinValue && position <= posMaxValue;
        ...
        Console.WriteLine($"The number {number} (Binary {numberBinary}) on bit position {position} from right to left has the value of {result}.");
    }
}